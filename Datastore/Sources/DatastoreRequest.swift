...
final class GoogleCloudDatastoreRequest: GoogleCloudAPIClient {
    ...
    func send<T: Codable>(
        ...
        guard (200...299).contains(response.status.code) else {
            do {
                throw try decoder.decode(CloudDatastoreAPIError.self, from: responseData)
            } catch {
                throw CloudDatastoreAPIError(error: CloudDatastoreAPIErrorBody(status: .unknownError,
