...
public struct CredentialsResolver {
    ...
    static func autoResolveCredentials(
        client: HTTPClient
    ) async throws -> ResolvedCredentials {
        
        for strategy in [
            CredentialsLoadingStrategy.environment,
            .environmentJSON,
            .computeEngine(client: client)
        ] {
            if let credentials = try? await Self.resolveCredentials(
                strategy: strategy,
                client: client
            ) {
                print("Using \(strategy) credentials")
                return credentials
            }
        }
        
        throw CredentialLoadError.automaticCredentialInitializationFailed
    }
    ...
    public static func resolveCredentials(
        ...
        switch strategy {
            ...
            case .autodetect:
                return try await Self.autoResolveCredentials(client: client)
