...
struct GoogleCloudServiceAccountCredentialsRequest: GoogleCloudAPIClient {
    ...
    func send<T: Codable>(method: HTTPMethod,
        ...
        guard (200...299).contains(response.status.code) else {
            do {
                throw try decoder.decode(ServiceAccountCredentialsAPIError.self, from: responseData)
            } catch {
                throw ServiceAccountCredentialsAPIError(error: ServiceAccountCredentialsAPIErrorBody(status: .unknownError,
